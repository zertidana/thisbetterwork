<template>
  <div class="container sports-page">
    ...
    <div class="row">
      <div class="col-md-6 mb-3" v-for="(news, index) in news" :key="index">
        <div class="card">
          <img :src="news.image" :alt="news.title" class="card-img-top">
          <div class="card-body">
            <h5 class="card-title">{{ news.title }}</h5>
            <p class="card-text">{{ news.preview }}</p>
            <div class="comment-section" v-if="news.showFullArticle">
              <textarea v-model="news.comment" placeholder="Add a comment..."></textarea>
...
export default defineComponent({
  setup() {
    ...
    onMounted(async () => {
      try {
        const response = await axios.get('/api/news?categoryTitle=Sports');
